 ...
 *                           3         5
 * 
 * So if we look at 2 chilren of 8 i,e 3,5 their sum is 8, now two children of 20 are 8 & 12 and its sum is 20.
 * So o/p is yes.
 * 
 ...
 * O/p No
 * 
 * Sum of children of parent 2 is 1+2 = 3 howverevr its not equal to parent.
 * 
 * Ex: null O/p yes
 * 
 * Ex:     5 o/p yes
 ...
 * If there then its sum should be equal to the root.
 ...
 * 
 * We keep checking and dequeing the parent while enqueing the child.
 * If at any point we dont find conditions satisfying we simply return false.
 * 
     */
    childrenSumProperty(root: TreeNode | null): boolean {
 ...
 *                           3         5
 * 
 * So for root 20 we have a sum varibale initialize as zero.
 ...
 * 
 * Now we have to recursively check for left subTree and right subTree howvere 
 ...
 * If root is null then also return true.
     * 
     */
    childrenSumProperty1(root: TreeNode | null): boolean {
        if (root === null) return true;
        if (root.left === null && root.right === null) return true;
        let sum = 0;
        if (root.left !== null) sum += root.left.data;
        if (root.right !== null) sum += root.right.data;
        return (root.data === sum && this.childrenSumProperty1(root.left) && this.childrenSumProperty1(root.right));
    }
}
...
console.log(root.childrenSumProperty(root));
