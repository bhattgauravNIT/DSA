 ...
 * 
 * An arr[i] is said to be an equilibrium point if the sum of the arry to the left of it is equal to the sum of
 * the array to the right of ity.If yes return true else return false.
 * 
 * Ex: [3,4,8,-9,20,6]
 * O/p is true
 * as 3+4+8+(-9) = 6
 * and last elemnet is also 6 so 20 is the equilibrium point.
 */
 ...
 * 
 * Similary maintain a right sum array i,e suffix sum
 * 
 * rSumSuffixSum = [ 32, 29, 25, 17, 26, 6 ]
 * 
 * Now at the index at which the leftSumPrefix array elemnet is equal to the right Sum suffix array element- at same index
 * is the index at which equilibrium point is present in nums array.
 * If there is no such element in leftSumPrefix which is equal to rightSumSuffix elemnet at that same index there is no such
...
function equilibriumPoint(nums) {
    ...
    return res === -1 ? `No equilibruim point` : nums[res];
}
 ...
 * 
 * So intead of maintaining two different array we can use two different varibale which will do the job.
*/
function equilibriumPoint(nums) {
