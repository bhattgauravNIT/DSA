/**Given the root of a tree, the task is to print top view of tree
 * 
 * For ex: 
 *                               10
 *                          20        30
 ...
 * 10 and 15 are the nodes at next vertical line vertical line but 10 comes first from top so 10
 ...
 * 10 and 20 are the nodes at next vertical line but 10 comes first so we print 10
 * 25 and 75 are the nodes at next vertical line but 25 comes first or is at top so 25
 ...
 * 20 is only next to its previous vertical line so 20
 * 10 ,18,15 where 18 is left child of 30 and 15 is right child of 20 are the nodes at next vertical but 10 comes at top.
...
class Tree {
     ...
     * 
     * In this problem we have to find the top view of a given binary tree.
     * Now firstly lets understand how can we identify that nodes are in same vertical aspect or line because the top view will simply be the
     * every first node on each vertical level. 
     * 
 ...
 * 
     ...
     * 30
     * 
     * So every first node or every top node for every level is:
     * 
     * 5
     * 20
     * 10
     * 30
     * 
     * and thus its the top view of the tree.
     * 
     *
 ...
 *                                
     ...
     * 
     * So if the horizontal distance which we are getting is already present in the map, this means that the top node of that vertical level is being already
     * processed and inserted in the map for that horizontal distance thus we don't do anything in this case.
     * 
     * but if the horizontal distance which we are getting is already not present in map this means that current node which we are processing is the top node or
     * the first node in the vertical level and we simply insert the key into map against the current node. 
     * 
     * Now we got a map but our requirement is to give the top view starting from left side. Thus we need to sort the map based on keys in ascending order.
     * 
...
root1.right = new Tree(50);

root1.left.left = new Tree(30);
root1.left.right = new Tree(40);
...
root3.left.right = new Tree(4);
root3.left.right.right = new Tree(5);
root3.topView(root3);