 ...
 * visisted or not while doing the bfs traversal of the graph but there is one corner case which is very imp.
 * 
 ...
 * 
 * So in case 0 we push it to queue , mark it as visited then
 * 
 * queue was not empty so we went inside and iterate over the adjacenecy list of 0, we found 1, 1 was not visited
 * so we mark it as visited and push it to queue and updated the parent of 1 as 0.
 * 
 * Now queue is having [1] and parent array looks like [-1,0,-1,-1] with 0 being parent of 1.
 * 
 * Now again queue is not empty so we pop from queue and print it, now we travserse over the adjacenecy list of 1
 * which is [0,2,3]
 * 
 ...
 * Now we went to 2 in adjacency list of 1 and 2 was not visited so we mark 2 as visited and push to queue and mark parent of 2 as 1,
 ...
 * 
    ...
    bfsTraversal(vertex: number): boolean {
        let q = new Queue();
        q.push(vertex);
